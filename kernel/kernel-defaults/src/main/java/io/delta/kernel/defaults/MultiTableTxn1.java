...
public class MultiTableTxn1 {
  ...
  private static void runRavenExp(Map<String, String> expConfigs) {
    ...
    String logFileName = String.format(Locale.getDefault(),"%s/multitabletxn1-delta-raven-%d-%d-log.json",
  ...
  private static void runVanillaExp(Map<String, String> expConfigs) {
    ...
    String expResultDir = expConfigs.get("exp_result_dir");
    String logFileName = String.format(Locale.getDefault(),"%s/multitabletxn1-delta-vanilla-%d-%d-log.json",
            expResultDir, numTables, numRowsPerFile);
    String summaryFileName = String.format("%s/summary.json", expResultDir);
    MetricsExporter.exportMetricsToLog(logFileName);
    MetricsExporter.appendSummaryToJson(summaryFileName);
    ...
    runTaskForDuration(running -> {
      ...
      while (running.get()) {
        ...
        for (int i = 0; i < tableIdxList.size();i++) {
          StructType schema = TPCDSSchema.SCHEMA_LIST.get(tableIdxList.get(i));
          ...
          CloseableIterator<FilteredColumnarBatch> physicalData = Transaction.transformLogicalData(engine,
                  txnStates.get(i), data, Collections.emptyMap());
          // Get the write context
          DataWriteContext writeContext = Transaction.getWriteContext(engine, txnStates.get(i), Collections.emptyMap());
          ...
          try {
            ...
            for (DataFileStatus dataFileStatus : dataFiles) {
              fileLogs.get(i).add(new File2(dataFileStatus.getPath(), File2.File2Type.ADD, "data"));
            }

            CloseableIterator<Row> dataActions = Transaction.generateAppendActions(engine, txnStates.get(i),
